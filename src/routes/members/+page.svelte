<script lang="ts">
	...
	import type { MemberWithMembership, PaginatedMembersResponse } from '$lib/models/member_with_membership';

    let membersData: MemberWithMembership[] = [];
    let totalItems = 0;
    let totalPages = 1;
    let currentPage = 1;
    let pageSize = 20;
    ...
    async function fetchMembers(pageToFetch = currentPage, query = debouncedSearchQuery) {
        ...
        try {
            ...
            const result = await invoke<PaginatedMembersResponse>('get_members_with_memberships_paginated', { payload });
    ...
    function handleAddNewMember() {
        goto('/members/new'); // Assuming you have this route
    }

    function handleEditMember(memberId: number) {
        goto(`/members/${memberId}`); // Assuming dynamic route for editing
    }
    ...
    function getMembershipStatusBadgeVariant(status: string | null): "default" | "secondary" | "destructive" | "outline" {
...
<div class="space-y-6">
    ...
    {#if isLoading && membersData.length === 0} <!-- Show skeletons only on initial load -->
        <Card.Root>
    ...
    {:else if error}
        <Card.Root class="border-destructive">
            <Card.Header><Card.Title class="text-destructive">Error Loading Members</Card.Title></Card.Header>
            <Card.Content>
                <p>{error}</p>
                <Button on:click={() => fetchMembers(currentPage, debouncedSearchQuery)} variant="outline" class="mt-4">Try Again</Button>
            </Card.Content>
        </Card.Root>
    {:else if membersData.length === 0 && !isLoading}
        <Card.Root>
            <Card.Content class="pt-6 text-center">
                ...
                {#if !debouncedSearchQuery}
                    <Button on:click={handleAddNewMember} variant="link" class="mt-2">Add the first member!</Button>
                {/if}
            </Card.Content>
        </Card.Root>
    {:else}
                ...
                <Table.Body>
                    {#each membersData as member (member.id)}
                        <Table.Row>
                            ...
                            <Table.Cell class="hidden md:table-cell text-muted-foreground">{member.card_id || 'N/A'}</Table.Cell>
                            <Table.Cell class="hidden lg:table-cell text-muted-foreground">{member.email || 'N/A'}</Table.Cell>
                            <Table.Cell>
                            ...
                            <Table.Cell class="text-center">
                                <Button on:click={() => handleEditMember(member.id)} variant="outline" size="icon" title="Edit Member">
